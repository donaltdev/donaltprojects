...
use LutekRest\LutekDev\LutekLibs\Menus\EasyInventoryAPI\task\DelayTask;
...
class DoubleChestInventory extends ChestInventory {
	...
	public function __construct(LutekRest $main) {
		$this->main = $main;
	...
	public function onClose(Player $player) : void {
		// Real block
		$packet = UpdateBlockPacket::create(BlockPosition::fromVector3($this->holder->add(1, 0, 0)), RuntimeBlockMapping::getInstance()->toRuntimeId($player->getWorld()->getBlock($this->holder)->getFullId()), UpdateBlockPacket::FLAG_NETWORK, UpdateBlockPacket::DATA_LAYER_NORMAL);
	...
	public function send(Player $player) {
		...
		// Fake block right
		$packet = UpdateBlockPacket::create(BlockPosition::fromVector3($this->holder->add(1, 0, 0)), RuntimeBlockMapping::getInstance()->toRuntimeId(VanillaBlocks::CHEST()->getFullId()), UpdateBlockPacket::FLAG_NETWORK, UpdateBlockPacket::DATA_LAYER_NORMAL);
		$player->getNetworkSession()->sendDataPacket($packet);
		...
		$packet = BlockActorDataPacket::create(BlockPosition::fromVector3($this->holder->add(1, 0, 0)), new CacheableNbt($nbt));
		$player->getNetworkSession()->sendDataPacket($packet);
